...
3. Eve sets the block.timestamp to a number in the future that is divisible by 15 and finds the target block hash.
...
contract Roulette {
  ...
  function spin() external payable {
    ...
    require(block.timestamp != pastBlockTime); // must wait for next block

    pastBlockTime = block.timestamp; // set pastBlockTime to current block timestamp
