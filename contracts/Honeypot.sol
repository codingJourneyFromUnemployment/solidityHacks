...
Bank.withdraw() is vulnerable to the reentracy attack.
So a hacker tries to drain Ether from Bank.
But actually the reentracy exploit is a bait for hackers.
By deploying Bank with HoneyPot in place of the logger, this contract becomes a trap for hackers. Let's see how.

1. Alice deploys HoneyPot
2. Alice deploys Bank with the address of HoneyPot
3. Alice deposits 1 Ether into Bank.
4. Eve discovers the reentracy exploit in Bank.withdraw and decides to hack it.
...
// Hacker tries to drain the Ethers stored in Bank by reentracy.
contract Attack {
  Bank bank;
...
// Let's say this code is in a separate file so that others cannot read it.abi
contract HoneyPot{
  function log(address _caller, uint _amount, string memory _action) public {
    if(equal(_action, "Withdraw")){
      revert("HoneyPot: Reentrancy detected");
    }
  }
